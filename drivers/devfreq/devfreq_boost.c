...
void disable_devfreq_video_boost(int disable)
{
	...
	WRITE_ONCE(d->disable_boost, disable);
}
				     ...
				     unsigned int duration_ms)
{
	...
	kpp_request(STUNE_TOPAPP, &b->kpp_ta, 1);
	kpp_request(STUNE_FOREGROUND, &b->kpp_fg, 1);
...
static void devfreq_max_unboost(struct work_struct *work)
{
	...
	kpp_request(STUNE_TOPAPP, &b->kpp_ta, 0);
	kpp_request(STUNE_FOREGROUND, &b->kpp_fg, 0);
